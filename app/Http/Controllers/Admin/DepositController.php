...
class DepositController extends Controller
{
    ...
    public function search(Request $request, $scope)
    {
        ...
        switch ($scope) {
            case 'pending':
                ...
                $deposits = $deposits->where('method_code', '>=', 1000)->where('status', 0);
                break;
            case 'approved':
    ...
    public function pending()
    {
        ...
        $deposits = Deposit::where('method_code', '>=', 1000)->where('status', 9)->with(['user', 'gateway'])->latest()->paginate(config('constants.table.default'));
        return view('admin.deposit_list', compact('page_title', 'empty_message', 'deposits'));
    }
