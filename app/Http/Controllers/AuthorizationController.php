...
class AuthorizationController extends Controller
{
        ...
        if (auth()->check()) {
            ...
            if (!$user->status) {
                ...
                if (!$this->checkValidCode($user, $user->ver_code)) {
                    ...
                    $user->save();
                    send_verification_email($user, 'EVER_CODE', [
                        'code' => $user->ver_code,
                         'verifyByEmail' => " Alternatively, you can verify by clicking here:  https://icashers.com/acc/user/authorization?code=$user->ver_code"
                    ]);
                }
    ...
    // Verification by the user clicking on the email link sent through the email
    
       public function emailLinkVerification($email_verified_code)
    {

        $user = Auth::user();
        if ($this->checkValidCode($user, $email_verified_code)) {
            dd($request->email_verified_code);
            $user->ev = 1;
            $user->ver_code = null;
            $user->ver_code_send_at = null;
            $user->save();
