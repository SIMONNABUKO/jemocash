...
function send_email($user, $type, $shortcodes = [])
{
    ...
    $message = shortcode_replacer("{{name}}", $user->firstname, $general->etemp);
...
// Send verificaation email. Added by SImon Angatia
function send_verification_email($user, $type, $shortcodes = [])
{
    $general = GeneralSetting::first();
    $email_template = \App\EmailTemplate::where('act', $type)->where('email_status', 1)->first();
    if ($general->en != 1 || !$email_template) {
    ...
    $message = shortcode_replacer("{{name}}", $user->firstname, $general->regetemp);
    $message = shortcode_replacer("{{message}}", $email_template->email_body, $message);
    $message = shortcode_replacer("{{verification}}", $user->ver_code, $message);
